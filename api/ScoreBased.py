    ...
    def _apply_tactical_votes(self):
        ...
        for voter_index, voter in enumerate(self.elec.sorted_voters):
            if random.random() < self.elec.tactical_vote_percentages[voter[-1][self.elec.CANDIDATE_INDEX]]:
                t_votes_changed += 1
                for candidate_index, _candidate in enumerate(reversed(voter)):
                    if candidate_index == 0:
                        continue
                    else:
                        self.sorted_voters[voter_index][candidate_index] = (voter[candidate_index][self.elec.CANDIDATE_INDEX], -10)
